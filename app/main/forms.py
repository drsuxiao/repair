...
class RepairRegistrationForm(FlaskForm):
    ...
    equipment_code = StringField('设备编号', validators=[DataRequired(), Length(min=8, max=10)],
...
class RepairConfirmForm(FlaskForm):
    ...
    # SelectField 这里务必注意coerce选项的添加，否则提交时，下拉表单中的内容无法通过validate_on_submit的 验证
    # Text Field类型，文本输入框，必须输入是"年-月-日"格式的日期
    repair_date = StringField('报修日期', validators=[DataRequired()], render_kw={'class': "form-control"})
    dept_code = SelectField('报修科室', validators=[DataRequired()], coerce=str, render_kw={'class': "form-control"})  # 报修科室
    repair_registrant = StringField('报修人', validators=[DataRequired()], render_kw={'class': "form-control"})  # 报修人
    brand_code = SelectField('设备品牌', validators=[DataRequired()], coerce=str, render_kw={'class': "form-control"})  # 设备品牌
    type_code = SelectField('设备类型', validators=[DataRequired()], coerce=str, render_kw={'class': "form-control"})  # 设备类型
    equipment_code = StringField('设备编号', validators=[DataRequired(), Length(min=8, max=10)], render_kw={'class': "form-control"})   # 设备编号
    fault_code = SelectField('设备故障', validators=[DataRequired()], coerce=str, render_kw={'class': "form-control"})  # 设备故障
    com_code = SelectField('维修公司', validators=[DataRequired()], coerce=str, render_kw={'class': "form-control"})  # 维修公司
    repair_man = StringField('维修人', validators=[DataRequired()], render_kw={'class': "form-control"})  # 维修人
    repair_confirm_date = StringField('维修归还日期', render_kw={'class': "form-control"})  # 维修确认日期
...
class EquipmentReturnForm(FlaskForm):
    ...
    设备归还科室确认表单
    """
    equipment_return_date = StringField('设备归还科室日期', validators=[DataRequired()], render_kw={'class': "form-control"})  # 设备归还科室日期
    equipment_return_man = StringField('设备归还科室人', validators=[DataRequired()], render_kw={'class': "form-control"})  # 设备归还科室人
...
class RepairReturnForm(FlaskForm):
    ...
    维修归还确认表单
    """
    repair_return_date = StringField('维修归还日期', validators=[DataRequired()], render_kw={'class': "form-control"})  # 维修归还日期
    repair_return_man = StringField('维修归还人', validators=[DataRequired()], render_kw={'class': "form-control"})  # 维修归还人


class BaseDataSetForm(FlaskForm):
    code = StringField('编码', validators=[DataRequired(), Length(min=3, max=10)], render_kw={'class': "form-control"})
